...
**コアバリュー**:
- 人間がやることは **仕訳の入力のみ**
- 入力後はワンクリックで e-Tax・eLTax への電子申告まで全自動
- freee より安いコスト
...
送信方式:       Pattern B（e-Tax/eLTaxへのAPIダイレクト送信）
...
### 送信方式: Pattern B（ダイレクトAPI）
商業会計ソフトを経由せず、自社システムから e-Tax/eLTax のAPIに直接送信する。

### e-Tax（国税: 法人税・消費税）
- 仕様書: **申請中**（e-tax-api@nta.go.jp にメール済み）
- ファイル形式: `.xtx`（XML）
- 現状: XMLスタブのみ実装済み（`etaxFileFormat.ts` の `generateEtaxXmlStub()`）
- 電子署名: PKCS#12（`.p12`ファイル）で自動署名

### eLTax（地方税: 法人住民税・事業税）
- 仕様書: **申請予定**（地方税共同機構 0570-081459 or Webフォーム）
- 現状: ドラフトCSVを県分・市分の2ファイルで出力（`generateEltaxDrafts()`）
- 申告書: 第六号様式（都道府県民税）/ 第二十号様式（市町村民税）

### 電子証明書
- **未取得**（ユーザーアクション待ち）
- 推奨: 法務局「商業登記電子証明書」（約9,000円/年）
- 証明書なしでも申告は受理されるが E.150 エラーが記録される
...
1. e-Tax XMLの正式実装（`etaxFileFormat.ts`）
2. eLTax APIへの直接送信実装（`eltaxFileFormat.ts`）
3. PKCS#12電子署名の実装（サーバーサイドPython or Node）
4. 受信通知の自動取得・Supabase保存

---

## 9. 重要な設計判断

### なぜ Pattern B（ダイレクト送信）を選んだか
- 商業会計ソフト（freee等）を経由しない → コスト削減の核心
- e-Tax/eLTax APIに直接接続 → 仕様書の取得が必要

### 電子証明書なしでも申告できるか
**できる**。E.150エラー（証明書未添付）が記録されるが申告自体は受理される。実際にごえん合同会社の第1期申告でも受理確認済み（受付番号: R1-2026-XXXXXXXX, R1-2026-XXXXXXXX）。
