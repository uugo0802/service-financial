...
export const EXPENSE_RULES: CategoryRule[] = [
  ...
  { pattern: /所得税|住民税/, account: "租税公課", taxCategory: "不課税", personalDeductionOnly: true, note: "所得税・住民税の納付は事業主個人の家事費であり、事業の必要経費には算入できません" },
  ...
  {
    // 外注費は給与と異なり多くの場合課税仕入となるため、給与と同じ不課税で自動確定せず、
    // インボイス登録の有無で控除税額が変わることを踏まえ「要確認」として人間レビューに回す。
    pattern: /外注費(?!.{0,4}消費税)/,
    account: "給料賃金/外注工賃",
    taxCategory: "要確認",
