...
// 貸借対照表・注記表は期首残高や資本構成などこのアプリが保持していない情報が
// 必要なため、意図的にここでは生成していない（貸借対照表は balanceSheetForm.ts、
// 株主資本等変動計算書は equityChangeForm.ts を参照。いずれも期首残高等の
// 不足情報を利用者に手入力してもらうことで下書き・試算を生成する）。
// ------------------------------------------------------------------
