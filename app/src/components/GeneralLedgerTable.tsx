    ...
    <div className="flex flex-col gap-4">
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="border border-stone-300 bg-white rounded-lg p-4">
          <p className="text-xs text-stone-500">前期繰越</p>
          <p className="text-xl font-semibold tabular-nums mt-1">
            <Yen v={result.openingBalance} />
          </p>
        </div>
        <div className="border border-stone-300 bg-white rounded-lg p-4">
          <p className="text-xs text-stone-500">借方合計</p>
          <p className="text-xl font-semibold tabular-nums mt-1">
            <Yen v={result.totalDebit} />
          </p>
        </div>
        <div className="border border-stone-300 bg-white rounded-lg p-4">
          <p className="text-xs text-stone-500">貸方合計</p>
          <p className="text-xl font-semibold tabular-nums mt-1">
            <Yen v={result.totalCredit} />
          </p>
        </div>
        <div className="border border-stone-300 bg-white rounded-lg p-4">
          <p className="text-xs text-stone-500">期末残高</p>
          <p className="text-xl font-semibold tabular-nums mt-1">
            <Yen v={result.closingBalance} />
          </p>
          <p className="text-xs text-stone-400 mt-1">仕訳 {result.rows.length}件</p>
        </div>
      </div>
            ...
            <table className="w-full text-sm border-collapse min-w-[640px]">
              ...
              <tbody>
                <tr className="border-b border-stone-200 last:border-b-0 bg-stone-50/60">
                  <td className="py-2 px-3 text-xs text-stone-500" colSpan={5}>
                    前期繰越
                  </td>
                  <td className="py-2 px-3 text-right tabular-nums text-xs text-stone-500">
                    <Yen v={result.openingBalance} />
                  </td>
                </tr>
                {result.rows.map((row) => (
                  <tr key={row.id} className="border-b border-stone-200 last:border-b-0">
                    ...
                    <td className="py-2.5 px-3 text-right tabular-nums whitespace-nowrap">
                      {row.debit > 0 ? <Yen v={row.debit} /> : <span className="text-stone-300">－</span>}
                    </td>
                    <td className="py-2.5 px-3 text-right tabular-nums whitespace-nowrap">
                      {row.credit > 0 ? <Yen v={row.credit} /> : <span className="text-stone-300">－</span>}
                    </td>
