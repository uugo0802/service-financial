...
const URGENCY_BADGE_CLASS: Record<DeadlineUrgency, string> = {
  ...
  normal: "border-stone-400 bg-stone-50 text-stone-600 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300",
};
...
export function WeeklyDigestPreview({ digest }: { digest: WeeklyDigest }) {
  ...
  return (
    <div className="border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-900 rounded-md overflow-hidden">
          ...
          <DigestSection title="AIカテゴライズのレビュー待ち">
            {digest.pendingReviewCount === 0 ? (
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-sm text-stone-500 dark:text-stone-400">件の取引が確認待ちです</span>
              </div>
            )}
          </DigestSection>
                ...
                {digest.upcomingDeadlines.map((d) => (
                  <li key={d.id} className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <span className="text-xs text-stone-400 dark:text-stone-500 ml-2 tabular-nums">{d.dueDate}</span>
                    </div>
          ...
          <DigestSection title="銀行残高が未突合の取引">
            {digest.unreconciledCount === 0 ? (
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-sm text-stone-500 dark:text-stone-400">件の取引が未突合です</span>
              </div>
            )}
          </DigestSection>
        </div>
...
function DigestSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="px-5 py-4">
      <h3 className="text-xs font-semibold text-stone-500 dark:text-stone-400 mb-2">{title}</h3>
      {children}
    </div>
  );
}
...
function EmptyRow({ text }: { text: string }) {
  return <p className="text-sm text-stone-500 dark:text-stone-400">{text}</p>;
}
