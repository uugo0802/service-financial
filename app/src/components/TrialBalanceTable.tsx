...
function TrialBalanceRow({ line }: { line: TrialBalanceLine }) {
  return (
    <tr className="border-b border-border/60 last:border-0 print:break-inside-avoid">
...
export function TrialBalanceTable({ tb }: { tb: TrialBalance }) {
  return (
    <section>
      ...
      <div className="overflow-x-auto border border-border bg-surface">
        <table className="w-full text-sm">
          <thead>
            ...
            <tr className="border-b border-stone-300 dark:border-stone-700 text-right text-muted-foreground text-xs">
      ...
      <p className={`text-xs mt-3 ${tb.balanced ? "text-muted-foreground" : "text-red-700"}`}>
        {tb.balanced
          ? "検算: 残高の借方合計＝貸方合計（一致）"
