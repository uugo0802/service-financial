...
import { buildSampleTransactions } from "@/app/dashboard/sampleData";
import { DashboardWidgetId } from "@/lib/dashboard/widgetLayout";

// dashboard/page.tsxと settings/appearance のウィジェット並び替えプレビュー
// （DashboardWidgetOrderEditor）の両方から、まったく同じ実データ・実チャート
// コンポーネントを使い回すための共有フック。データ計算ロジックを二重管理しないため、
// widgetSectionsの構築自体はここに一本化する。
...
export interface DashboardWidgetSectionsResult {
  /** データが1件もなく、ウィジェットを何も描画できない状態か */
  isEmpty: boolean;
  isSampleData: boolean;
  /** おすすめサービス（パートナー紹介）バナー用のカテゴリ推定結果 */
  partnerCategories: ReturnType<typeof recommendPartnerCategories>;
  /** 各ウィジェットIDに対応する実際の描画内容（Cardでラップ済み） */
  widgetSections: Partial<Record<DashboardWidgetId, ReactNode>>;
}

export function useDashboardWidgetSections(): DashboardWidgetSectionsResult {
  ...
  if (yearlyTrend.length === 0) {
    return { isEmpty: true, isSampleData, partnerCategories, widgetSections: {} };
  }
  ...
  return { isEmpty: false, isSampleData, partnerCategories, widgetSections };
}
